// Arquivo: wokwi_firmware_modified.ino
// Projeto: Monitoramento e Controle da Qualidade do Ar com ESP32 e MQTT
// Versão modificada: adiciona 3 níveis de alerta (OK / RUIM / MUITO RUIM)
// Autor: Manus (Assistente) - ajustado para Márcia
...
// Dois limites para os três níveis
int threshold_ppm = 150;   // limite entre OK e RUIM
int threshold_ppm2 = 300;  // limite entre RUIM e MUITO RUIM
...
void loop() {
  ...
  if (now - lastMsg > 5000) { // Publica a cada 5 segundos
    ...
    control_actuator(ppm_value);             // Liga/desliga exaustor (com 3 níveis)
  }
}
...
void control_actuator(int ppm) {
  // Usa dois limites para diferenciar 3 níveis
  if (ppm <= threshold_ppm) {
    digitalWrite(PIN_RELE, LOW);
    Serial.println("Qualidade do ar OK. Exaustor DESLIGADO.");
  } else if (ppm > threshold_ppm && ppm <= threshold_ppm2) {
    // Nível RUIM: apenas alerta (não aciona o exaustor automaticamente)
    digitalWrite(PIN_RELE, LOW);
    Serial.println("⚠️ ALERTA: Qualidade do ar RUIM. Exaustor DESLIGADO.");
  } else {
    // Nível MUITO RUIM: acionar exaustor
    digitalWrite(PIN_RELE, HIGH);
    Serial.println("⚠️ ALERTA: Qualidade do ar MUITO RUIM. Exaustor LIGADO.");
  }
}
